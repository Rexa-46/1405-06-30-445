...
function SmileFrownArc({ income, expense }) {
  ...
  const w = 240, h = 120, cx = 120, cy = 60, r = 72;
  const frac = Math.min(Math.abs(net) / Math.max(income, expense, 1), 1);
  const endAngle = Math.PI * frac;
  const start = net > 0 ? Math.PI : 0;
  const endA = net > 0 ? Math.PI - endAngle : endAngle;
  const x1 = cx + r * Math.cos(start), y1 = cy + r * Math.sin(start);
  const x2 = cx + r * Math.cos(endA), y2 = cy + r * Math.sin(endA);
  const d = net === 0 ? "" : `M ${x1} ${y1} A ${r} ${r} 0 0 ${net > 0 ? 0 : 1} ${x2} ${y2}`;
  return <div style={{ textAlign: "center" }}>
    <svg viewBox={`0 0 ${w} ${h}`} width="100%" height="120" style={{ maxWidth: 300 }}>
      <path d={`M ${cx-r} ${cy} A ${r} ${r} 0 0 1 ${cx+r} ${cy}`} fill="none" stroke={t.border} strokeWidth="16" strokeLinecap="round" />
      {d && <path d={d} fill="none" stroke={net > 0 ? BRAND.darkgreen : BRAND.crimson} strokeWidth="16" strokeLinecap="round" />}
      <line x1={cx-r-10} y1={cy} x2={cx+r+10} y2={cy} stroke={t.sub} strokeWidth="2" />
    </svg>
    <div style={{ fontSize: 12, fontWeight: 800, color: net > 0 ? BRAND.darkgreen : net < 0 ? BRAND.crimson : t.sub }}>
      {net > 0 ? "درآمد بیشتر از هزینه" : net < 0 ? "هزینه بیشتر از درآمد" : "تراز"} · {toFaInt(Math.abs(net))} ریال
    </div>
  </div>;
}
...
export default function App() {
  ...
  async function fetchRates() {
    try {
      const data = Capacitor.isNativePlatform()
        ? (await CapacitorHttp.get({ url: "https://open.er-api.com/v6/latest/USD" })).data
        : await (await fetch("https://open.er-api.com/v6/latest/USD")).json();
      const irr = Number(data?.rates?.IRR);
      if (!Number.isFinite(irr) || irr <= 0) throw new Error("no IRR rate");
      const fetchedAt = new Date().toISOString();
      setRates({ usd: irr, fetchedAt, source: "live" });
      setSettings((prev) => ({ ...prev, lastRatesUpdate: fetchedAt }));
    } catch {
      const manual = Number(String(settings.manualUsdRate || "").replace(/,/g, ""));
      setRates({ usd: manual > 0 ? manual : null, fetchedAt: new Date().toISOString(), source: manual > 0 ? "manual" : "failed" });
    }
  }
  ...
  const backupState = { accounts, categories, transactions, budgets, loans, checks, bills, assets, recurring, favorites, settings, shortcuts, members, events, projects, fiscalPeriods, notes, reminders };
  ...
  return (
    <ThemeCtx.Provider value={t}>
      <div dir="rtl" onClick={handleTripleTap} style={{ fontFamily: FONT, background: t.bg, color: t.text, minHeight: "100vh", maxWidth: 480, margin: "0 auto", display: "flex", flexDirection: "column", position: "relative", paddingBottom: "calc(84px + env(safe-area-inset-bottom, 0px))", boxShadow: "0 0 30px rgba(0,0,0,0.08)", zoom: settings.fontScale || 1 }}>
        <style>{`
...
function HomeView({
  ...
  function sectionBody(key) {
    switch (key) {
      case "shortcut":
        return shortcuts.length === 0 ? <EmptyRow text="میانبری ساخته نشده" /> : shortcuts.map((sc) => (
          <Row key={sc.id} title={sc.name} subtitle={`${categories.find((c) => c.id === sc.categoryId)?.name || "—"} · لمس برای ثبت سریع`} value="اجرای سریع" valueColor={BRAND.fab} onClick={() => onRunShortcut?.(sc)} />
        ));
      case "expense":
...
function ShortcutsManager({ shortcuts, setShortcuts, categories, accounts }) {
  const st = useStyles();
  const [form, setForm] = useState({ name: "", type: "expense", categoryId: "", accountId: accounts[0]?.id || "", note: "" });
  ...
  return <div>
    <div style={{ ...st.card, padding: 14, marginBottom: 16 }}>
      ...
      <select value={form.accountId} onChange={(e) => setForm({ ...form, accountId: e.target.value })} style={st.input}>
        <option value="">حساب پیش‌فرض</option>{accounts.map((a) => <option key={a.id} value={a.id}>{a.name}</option>)}
      </select>
    ...
    <div style={{ ...st.card, padding: "4px 12px" }}>
      ...
      {shortcuts.map((sc) => <Row key={sc.id} title={sc.name} subtitle={`${categories.find((c) => c.id === sc.categoryId)?.name || "—"} · ${accounts.find((a) => a.id === sc.accountId)?.name || "—"}`} leftIcon={<Star size={16} />} leftColor={BRAND.fab} extra={<button onClick={() => setShortcuts((p) => p.filter((x) => x.id !== sc.id))} style={{ background: "none", border: "none", color: BRAND.crimson }}><Trash2 size={15} /></button>} chevron={null} />)}
    </div>
  </div>;
}
...
function SideMenu({ onClose, setSubView, profileName }) {
  const items = [
    { label: "میانبرهای تراکنش", key: "shortcuts" },
    { label: "حساب‌ها و کارت‌ها", key: "accounts" },
    { label: "دسته‌بندی‌ها و برچسب‌ها", key: "categories" },
    { label: "بودجه‌بندی", key: "budgets" },
    { label: "تراکنش‌های تکرارشونده", key: "recurring" },
        ...
        <div style={{ background: BRAND.header, color: "#fff", padding: "18px 16px 22px", display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: "#d8c9e8" }}>Rexa · نسخه ۴.۷.۰</div>
            </div>
          </div>
        </div>
